...
intercepts for all respondents.)

Second, we calculate another sum of squared residuals generated by
allowing the predictions to vary by estimated LCAM trajectory. We divide
this number by the total sum of squares and subtract it from 1. We then
subtract \(V(D)\) from this value. This gives us a value we denote as
\(V(C)\), the additional proportion of variance accounted for by
intrapersonal change.
